    ...
    for(int i = 0; i< Tskim->GetEntries(); i++){
        ...
        if(Event > 0){
            ...
            if(lumi_skim != lumi_forest){
                cout << "Lumi mismatch at entry " << i << ": Skim lumi = " << lumi_skim << ", Forest lumi = " << lumi_forest << endl;
            }

            if(event_skim != event_forest){
                cout << "Event mismatch at entry " << i << ": Skim event = " << event_skim << ", Forest event = " << event_forest << endl;
            } 
        
            if(hiBin_skim != hiBin_forest){
                cout << "hiBin mismatch at entry " << i << ": Skim hiBin = " << hiBin_skim << ", Forest hiBin = " << hiBin_forest << endl;
            } 

            if(hiHF_skim != hiHF_forest){
                cout << "hiHF mismatch at entry " << i << ": Skim hiHF = " << hiHF_skim << ", Forest hiHF = " << hiHF_forest << endl;
            }

            ncollweight_forest = FindNColl(hiBin_forest);
            if(ncollweight_skim != ncollweight_forest){
                cout << "ncollweight mismatch at entry " << i << ": Skim ncollweight = " << ncollweight_skim << ", Forest ncollweight = " << ncollweight_forest << endl;
            }

            if(eventweight_skim != eventweight_forest){
                cout << "eventweight mismatch at entry " << i << ": Skim eventweight = " << eventweight_skim << ", Forest eventweight = " << eventweight_forest << endl;
            }

            if(pthat_skim != pthat_forest){
                cout << "pthat mismatch at entry " << i << ": Skim pthat = " << pthat_skim << ", Forest pthat = " << pthat_forest << endl;
            }
        }
        ...
        if(Vertex > 0){
            ...
            for (int m = 0; m < (vx_forest_vec->size()); m++) {
                if (BestVertex < 0 || vptsum_forest_vec->at(m) > vptsum_forest_vec->at(BestVertex)){BestVertex = m;}
            }
            ...
            if(vxerror_skim != vxerror_forest){
                cout << "vxerror mismatch at entry " << i << ": Skim vxerror = " << vxerror_skim << ", Forest vxerror = " << vxerror_forest << endl;
            }

            if(vyerror_skim != vyerror_forest){
                cout << "vyerror mismatch at entry " << i << ": Skim vyerror = " << vyerror_skim << ", Forest vyerror = " << vyerror_forest << endl;
            }

            if(vzerror_skim != vzerror_forest){
                cout << "vzerror mismatch at entry " << i << ": Skim vzerror = " << vzerror_skim << ", Forest vzerror = " << vzerror_forest << endl;
            }
        }
        ...
        if(nref > 0){
            ...
            for(int j = 0; j < nref; j++){
                ...
                if (JetPt_forest[j] < 30){continue;}
                if (fabs(JetEta_forest[j]) > 2){continue;}
                ...
                if(SVTX > 0){
                    ...
                    for(int k = 0; k < nsvtx_forest[j]; k++){
                        ...
                        if(svtxJetId_forest[k] != j){continue;}
                ...
                if(Track > 0){
                    ...
                    for(int l = 0; l < ntrk_forest[j]; l++){
                        ...
                        if(trkJetId_forest[l] != j){continue;}
                        trk_idx+=1;
                        ...
                        if((trkIp3dSig_skim->at(skim_idx).at(trk_idx) != trkIp3dSig_forest[l])&&(trkIp3dSig_forest[l] == trkIp3dSig_forest[l])){
                        ...
                        if((trkIpProb3d_skim->at(skim_idx).at(trk_idx) != trkIpProb3d_forest[l])&&(trkIpProb3d_forest[l] == trkIpProb3d_forest[l])){
                        ...
                        if((trkIpProb2d_skim->at(skim_idx).at(trk_idx) != trkIpProb2d_forest[l])&&(trkIpProb2d_forest[l] == trkIpProb2d_forest[l])){
                ...
                if(Muon > 0){
                    ...
                    for(int mu1idx = 0; mu1idx < nmuons; mu1idx++){
                        if(!isMuonSelected(mupt_forest, mueta_forest, IsTracker_forest, IsGlobal_forest, IsHybridSoft_forest, IsGood_forest, mu1idx)){continue;}
                        for(int mu2idx = mu1idx+1; mu2idx < nmuons; mu2idx++){
                            if(!isMuonSelected(mupt_forest, mueta_forest, IsTracker_forest, IsGlobal_forest, IsHybridSoft_forest, IsGood_forest, mu2idx)){continue;}
                            ...
                            float drjetmu1 = sqrt(DeltaPhi(JetPhi_forest[j], muphi1)*DeltaPhi(JetPhi_forest[j], muphi1) + (mueta1 - JetEta_forest[j])*(mueta1 - JetEta_forest[j]));
                            float drjetmu2 = sqrt(DeltaPhi(JetPhi_forest[j], muphi2)*DeltaPhi(JetPhi_forest[j], muphi2) + (mueta2 - JetEta_forest[j])*(mueta2 - JetEta_forest[j]));

                            if(drjetmu1 > 0.3 || drjetmu2 > 0.3){continue;}
                    ...
                    if(maxmumuPt > 0. && maxMu1Index >= 0 && maxMu2Index >= 0) {ismumutagged = true;}
                    ...
                    if(ismumutagged){
                        ...
                        if(mupt_forest->at(maxMu1Index) != muPt1_skim->at(skim_idx)){
                            cout << "muPt1 mismatch at entry " << i << ", jet " << j 
                                << ": Skim muPt1 = " << muPt1_skim->at(skim_idx) 
                                << ", Forest muPt1 = " << mupt_forest->at(maxMu1Index) << endl;
                        }
                        if(mupt_forest->at(maxMu2Index) != muPt2_skim->at(skim_idx)){
                            cout << "muPt2 mismatch at entry " << i << ", jet " << j 
                                << ": Skim muPt2 = " << muPt2_skim->at(skim_idx) 
                        ...
                        float drjetmu1_forest = sqrt(DeltaPhi(JetPhi_forest[j], muphi_forest->at(maxMu1Index)) * DeltaPhi(JetPhi_forest[j], muphi_forest->at(maxMu1Index)) + 
                                        (mueta_forest->at(maxMu1Index) - JetEta_forest[j]) * (mueta_forest->at(maxMu1Index) - JetEta_forest[j]));
                        float drjetmu2_forest = sqrt(DeltaPhi(JetPhi_forest[j], muphi_forest->at(maxMu2Index)) * DeltaPhi(JetPhi_forest[j], muphi_forest->at(maxMu2Index)) + 
                                        (mueta_forest->at(maxMu2Index) - JetEta_forest[j]) * (mueta_forest->at(maxMu2Index) - JetEta_forest[j]));

                        if(fabs(drjetmu1_forest - dRJetmu1_skim->at(skim_idx)) > 1.0e-6){
                            cout << "drjetmu1 mismatch at entry " << i << ", jet " << j 
                                << ": Skim drjetmu1 = " << dRJetmu1_skim->at(skim_idx) 
                                << ", Forest drjetmu1 = " << drjetmu1_forest << endl;
                        }

                        if(fabs(drjetmu2_forest - dRJetmu2_skim->at(skim_idx)) > 1.0e-6){
                            cout << "drjetmu2 mismatch at entry " << i << ", jet " << j 
                                << ": Skim drjetmu2 = " << dRJetmu2_skim->at(skim_idx) 
                                << ", Forest drjetmu2 = " << drjetmu2_forest << endl;
                        }
                        

                        TLorentzVector gMu1, gMu2;
                        gMu1.SetPtEtaPhiM(mupt_forest->at(maxMu1Index), mueta_forest->at(maxMu1Index), muphi_forest->at(maxMu1Index), M_MU);
                        gMu2.SetPtEtaPhiM(mupt_forest->at(maxMu2Index), mueta_forest->at(maxMu2Index), muphi_forest->at(maxMu2Index), M_MU);
                        ...
                        mudR_forest = sqrt(DeltaPhi(muphi_forest->at(maxMu2Index), muphi_forest->at(maxMu1Index)) * DeltaPhi(muphi_forest->at(maxMu2Index), muphi_forest->at(maxMu1Index)) + 
                                        (mueta_forest->at(maxMu1Index) - mueta_forest->at(maxMu2Index)) * (mueta_forest->at(maxMu1Index) - mueta_forest->at(maxMu2Index)));

                        if(mumumass_forest !=  mumumass_skim->at(skim_idx)){
                            cout << "mumuMass mismatch at entry " << i << ", jet " << j 
                                << ": Skim mumuMass = " << mumumass_skim->at(skim_idx) 
                                << ", Forest mumuMass = " << mumumass_forest << endl;
                        }

                        if(mumupt_forest != mumupt_skim->at(skim_idx)){
                            cout << "mumuPt mismatch at entry " << i << ", jet " << j 
                                << ": Skim mumuPt = " << mumupt_skim->at(skim_idx) 
                                << ", Forest mumuPt = " << mumupt_forest << endl;
                        }

                        if(mumueta_forest != mumueta_skim->at(skim_idx)){
                            cout << "mumuEta mismatch at entry " << i << ", jet " << j 
                                << ": Skim mumuEta = " << mumueta_skim->at(skim_idx) 
                                << ", Forest mumuEta = " << mumueta_forest << endl;
                        }

                        if(mumuphi_forest != mumuphi_skim->at(skim_idx)){
                            cout << "mumuPhi mismatch at entry " << i << ", jet " << j 
                                << ": Skim mumuPhi = " << mumuphi_skim->at(skim_idx) 
                                << ", Forest mumuPhi = " << mumuphi_forest << endl;
                        }

                        if(mumuY_forest != mumuY_skim->at(skim_idx)){
                            cout << "mumuY mismatch at entry " << i << ", jet " << j 
                                << ": Skim mumuY = " << mumuY_skim->at(skim_idx) 
                                << ", Forest mumuY = " << mumuY_forest << endl;
                        }
                                            
                        if(fabs(mudR_forest - mudR_skim->at(skim_idx)) > 1.0e-6){
                            cout << "mudR mismatch at entry " << i << ", jet " << j 
                                << ": Skim mudR = " << mudR_skim->at(skim_idx) 
...
void skimvalidator(){
    ...
    validate(
        1,       // Event properties (HiEvtAnalyzer)
