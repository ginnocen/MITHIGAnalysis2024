...
if [[ $# -ne 6 ]]; then
    echo "usage: ./tt-condor-checkfile.sh [executable file] [input dir] [output dir] [max jobs] [log dir] [IsData] " # [CollisionSystem] [Fraction] [ApplyTriggerRejection] [ApplyEventRejection] [ApplyTrackRejection] [sampleType] [DebugMode] [includeL1EMU]"
    exit 1
fi
