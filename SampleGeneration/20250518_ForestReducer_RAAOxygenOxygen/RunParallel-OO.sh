...
for FILEPATH in "$PATHSAMPLE"/HiForestMiniAOD*; do
    ...
    echo ./ProcessSingleFile-OO.sh "$FILEPATH" $COUNTER $OUTPUT $DOGENLEVEL $ISDATA $SAMPLETYPE $DEBUGMODE $INCLUDEPPSANDFSC $SAVETRIGGERBITS &
    ./ProcessSingleFile-OO.sh "$FILEPATH" $COUNTER $OUTPUT $DOGENLEVEL $ISDATA $SAMPLETYPE $DEBUGMODE $INCLUDEPPSANDFSC $SAVETRIGGERBITS &
