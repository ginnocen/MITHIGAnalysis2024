...
void doRatio(	TH1D* ref,
		const std::vector<TH1D*>& cmp_vec,
		 TPad* motherPad, TLegend* leg,
		 string ratioYAxisName = "Ratio",
		 bool logy = false,
		 string curvePlotStyle = "h",
		 double padSplit = 0.3)
{
	...
	TH1D* hempty_ratio = new TH1D(	Form("hempty_ratio_%s", ref->GetName()), 
					Form(";%s;%s", 
						ref->GetXaxis()->GetTitle(), ratioYAxisName.c_str()), 
						numBinsX, xMin, xMax);
...
double getChiSquare(TH1D* h1, TH1D* h2)
{
	...
	for (int i = 1; i <= h1->GetNbinsX(); ++i)
	{
		...
		if (denominator!=0)
		{
			...
			if (content1!=0 || content2!=0) std::cerr << "Warning: Zero denominator in bin " << i << std::endl;
		}
	}
	return (chiSquare);
}
...
int main(int argc, char *argv[])
{
	...
	for (const auto& [key, hist1] : v1.histDict)
	{
		...
		if (hist1->GetEntries()==0 && hist2->GetEntries()==0) continue;

		histNameToPlot.push_back(key);
	}

	int _nHist = histNameToPlot.size();
	int _nCol  = 5; int _nRow  = TMath::Ceil(_nHist/((float) _nCol));
	...
	for (int iPlot = 0; iPlot < histNameToPlot.size(); ++iPlot)
	{
		...
		doRatio(hist2, {hist1}, (TPad*) canv->GetPad(iPlot+1), 
						(iPlot==0)? legend: nullptr,
						Form("%s/%s", LegendName1.c_str(), LegendName2.c_str()),
						(	key=="VX" || key=="VXError" || key=="VY" || key=="VYError" || key=="VZ" || key=="VZError" ||
							key=="DsvpvDistance" || key=="DsvpvDisErr" || key=="DsvpvDistance_2D" || key=="DsvpvDisErr_2D" )? true: false);
	}
