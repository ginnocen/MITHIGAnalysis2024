void StudyNcNbHad(TString  myvar="Nc"){
  ...
  TFile f("/data/NewSkims_gtoccbar/SkimMC2018PbPb_Version20241201_InputForest_20241201_DiJet_pThat-15_TuneCP5_HydjetDrumMB_5p02TeV_Pythia8_PARTIAL.root");
  ...
  TString cuts[kCases] = {
                      ...
                      "mumuMass > 0. && fabs(muDiDxy1) > 0.01 && fabs(muDiDxy2) > 0.01 && fabs(muDiDxy1) < 0.05 && fabs(muDiDxy2) < 0.05 && JetPT > 100."
                     };
  TString titles[kCases] = {
		      "JetPT > 100 GeV",
		      "+ MuMu Tagged, DCAXY muons > 0.01 cm",
		      "+ DCAXY < 0.05 cm"
		     };
  ...
  for(int i = 0; i < kCases; i++){
    ...
    if (i==0) hNHFHad[i]->Draw();
    else hNHFHad[i]->Draw("same");
