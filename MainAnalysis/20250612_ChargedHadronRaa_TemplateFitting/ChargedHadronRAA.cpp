...
public:
  ...
  void analyze(Parameters &par) {
    outf->cd();
    hMult = new TH1D(Form("hMult%s", title.c_str()), "", 100, 0,200);
    hFEMax = new TH1D(Form("hFEMax%s", title.c_str()), "", 100, 0, 200);
    ...
    for (unsigned long i = 0; i < nEntry; i++) {
      ...
      hFEMax->Fill(max(MChargedHadronRAA->HFEMaxPlus, MChargedHadronRAA->HFEMaxMinus));
      Mult = 0;
      for (unsigned long j = 0; j < MChargedHadronRAA->trkPt->size(); j++) {
        ...
        if(MChargedHadronRAA->trkPt->at(j) > 1){Mult +=1;}
      }
      hMult->Fill(Mult);
    } // end of event loop
  } // end of analyze
