...
void RooUnfoldData(std::string file_mc="/home/cbaldene/Unfolding/RooUnfoldSetupForLuis/examples/response3.root", 
...
*/
  ...
  TFile* f_mc=new TFile(file_mc.c_str());
  TFile* f_data=new TFile(file_data.c_str());
  TH1D *pTjet_smeared=(TH1D*)f_data->Get("jetPtDimuon_0");
  pTjet_smeared->SetName("pTjet_smeared");
...
//   TMatrixT<double> covariance();
     ...
     for(int jar=0;jar< 15;jar++)
     {
	  ...
	  htempUnf->Divide(miss); 
     	  TH1D *htempFold=(TH1D*)hfold->Clone("htempFold");          
     	  htempFold->SetName(Form("Bayesian_Foldediter%d.root",iter));
	  pTjet_smeared->Chi2Test(htempFold, "P");
          covariance.Write();
...
#ifndef __CINT__
int main () { RooUnfoldData(); return 0; }  // Main program when run stand-alone
#endif
