...
#define DMASS_UNBINNEDFIT_NBINS 100
...
// One pass over MC nt for event-weight fractions (replaces 4x TChain::GetEntries full scans).
static void countMcGenInMassWindow(TChain *mctree, double DmassMin, double DmassMax,
                                   double &nsig, double &nswp, double &npkkk, double &npkpp,
                                   bool doPkkk, bool doPkpp) {
  nsig = nswp = npkkk = npkpp = 0.;
  TTreeReader reader(mctree);
  TTreeReaderValue<float> dmass(reader, "Dmass");
  TTreeReaderValue<float> dgen(reader, "Dgen");
  while (reader.Next()) {
    float m = *dmass;
    if (m < DmassMin || m > DmassMax)
      continue;
    int g = (int)(*dgen);
    if (g == 23333 || g == 41022 || g == 41044)
      nsig += 1.;
    else if (g == 23344 || g == 41122 || g == 41144)
      nswp += 1.;
    else if (doPkkk && g == 333 && m < 1.8648f)
      npkkk += 1.;
    else if (doPkpp && g == 333 && m > 1.8648f)
      npkpp += 1.;
  }
}
                ...
                int DmassNBins)
{
  ...
  // Import with Cut (avoids loading full MC into memory then reduce()).
  RooDataSet _data("_data", "dataset", RooArgSet(m, Dgen), Import(*mctree),
                   Cut(Form("((Dgen == 23333 || Dgen == 41022 || Dgen == 41044) || "
                            " (Dgen == 23344 || Dgen == 41122 || Dgen == 41144)) && "
                            " Dmass > %f && Dmass < %f",
                            DmassMin, DmassMax)));
  RooDataSet &data = _data;
              ...
              int DmassNBins)
{
  ...
  RooDataSet _data("_data", "dataset", RooArgSet(m, Dgen), Import(*mctree),
                   Cut(Form("Dgen == 333 && Dmass < 1.8648 && Dmass > %f && Dmass < %f", DmassMin,
                            DmassMax)));
  RooDataSet &data = _data;
              ...
              int DmassNBins)
{
  ...
  RooDataSet _data("data", "dataset", RooArgSet(m), Import(*datatree),
                   Cut(Form("Dmass > %f && Dmass < %f", DmassMin, DmassMax)));
  RooDataSet &data = _data;
  ...
  ((TH1D*)data.createHistogram("h_roofit", m, Binning(DmassNBins)))->Write();
...
//  bool doSyst_MassWin  = CL.GetBool  ("doSyst_MassWin", false); // vary the mass window
  ...
  if (neventsInput=="")
  {
    double nsig = 0., nswp = 0., npkkk = 0., npkpp = 0.;
    countMcGenInMassWindow(mctree, DmassMin, DmassMax, nsig, nswp, npkkk, npkpp, doPkkk, doPkpp);
  ...
  if (doPkpp && !(pipimcInputs.size()==1 && pipimcInputs[0].find(".dat")!=string::npos)) {
    ...
    pipimc_fit(pipimctree, rstDir, pkppdat, plotTitle.str(), DmassMin, DmassMax, DmassNBins);
  }
